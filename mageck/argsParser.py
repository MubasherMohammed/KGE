...
def arg_count(subparser):
  ...
  cnt_qcgroup.add_argument('--html-report',action='store_true',help='Generate a self-contained HTML report after counting.')
...
def arg_test(subparser):
  ...
  gengroup.add_argument('--html-report',action='store_true',help='Generate a self-contained HTML report after analysis.')
...
def arg_mle(subparser):
  ...
  mlegroup.add_argument('--html-report',action='store_true',help='Generate a self-contained HTML report after MLE analysis.')
...
def arg_report(subparser):
  """
  Adding arguments for the HTML report subcommand
  """
  subp_report=subparser.add_parser('report',help='Generate a self-contained HTML report from MAGeCK output files.')
  # required
  reqgroup=subp_report.add_argument_group(title='Required arguments',description='')
  reqgroup.add_argument('-n','--output-prefix',required=True,help='The prefix used in the original MAGeCK run (-n option). Auto-detects output files matching this prefix.')
  # optional input
  iogroup=subp_report.add_argument_group(title='Optional input arguments',description='')
  iogroup.add_argument('-k','--count-table',help='Path to the read count table (CSV or TSV). If not provided, auto-detected from the output prefix.')
  iogroup.add_argument('--gene-summary',nargs='+',help='One or more gene_summary.txt files (RRA or MLE). If not provided, auto-detected.')
  iogroup.add_argument('--sgrna-summary',nargs='+',help='One or more sgrna_summary.txt files. If not provided, auto-detected.')
  iogroup.add_argument('--design-matrix',help='Path to the design matrix file (for MLE reports).')
  iogroup.add_argument('--norm-method',choices=['none','median','total'],default='median',help='Normalization method for QC plots. Default: median.')
  # optional output
  outgroup=subp_report.add_argument_group(title='Optional output arguments',description='')
  outgroup.add_argument('-o','--output-file',help='Output HTML file path. Default: {prefix}.report.html.')
  outgroup.add_argument('--title',help='Report title. Default: "MAGeCK CRISPR Screen Report".')
  # optional plot
  plotgroup=subp_report.add_argument_group(title='Optional plot arguments',description='')
  plotgroup.add_argument('--fdr-threshold',type=float,default=0.05,help='FDR threshold for highlighting significant genes. Default: 0.05.')
  plotgroup.add_argument('--top-n',type=int,default=20,help='Number of top genes to display in tables and label on plots. Default: 20.')
  plotgroup.add_argument('--skip-qc',action='store_true',help='Skip the QC section.')
  plotgroup.add_argument('--skip-results',action='store_true',help='Skip the results section.')
  # enrichment
  enrgroup=subp_report.add_argument_group(title='Optional enrichment arguments',description='')
  enrgroup.add_argument('--enrichment-top-n',type=int,default=50,help='Number of top genes to use for pathway enrichment. Default: 50.')
  enrgroup.add_argument('--organism',default='human',help='Organism for hallmark gene sets (human or mouse). Default: human.')
  enrgroup.add_argument('--skip-enrichment',action='store_true',help='Skip the pathway enrichment section.')
  enrgroup.add_argument('--enrichment-fdr',type=float,default=0.05,help='FDR threshold for enrichment significance. Default: 0.05.')
